  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-semibold text-gray-900">Sotuvlar</h1>
        <button onClick={loadSales} className="flex items-center px-4 py-2 bg-gray-800 text-white rounded-lg">
          <RefreshCw size={16} className="mr-2" /> Yangilash
        </button>
      </div>
      <div className="bg-white rounded-xl shadow-sm border p-4 flex gap-4 items-center">
        <div className="flex-1 relative">
          ...
          <input
            ...
            placeholder="ID, mijoz yoki mahsulot bo'yicha qidiring"
        ...
        <table className="w-full min-w-[800px]">
          <thead className="bg-gray-50">
            <tr>
              ...
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Mijoz</th>
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Jami</th>
          ...
          <tbody className="divide-y">
            {filtered.map((t) => (
              <tr key={t.id} className="hover:bg-gray-50">
                ...
                <td className="px-6 py-3">{(t.total || 0).toLocaleString()} so'm</td>
                <td className="px-6 py-3">{new Date(t.createdAt).toLocaleString('uz-UZ')}</td>
              </tr>
            ))}
            {filtered.length === 0 && (
              <tr>
                <td colSpan={4} className="px-6 py-6 text-center text-gray-500">Ma'lumot yo'q</td>
              </tr>
            )}
          </tbody>
        </table>
