...
function Sotuvchilar() {
  ...
  const [selectedUserId, setSelectedUserId] = useState(''); // Empty means all salespeople
  ...
  // Set default date range to current day (00:00 to 23:59) with guard
  ...
  const fetchSalesData = async () => {
    ...
    try {
      ...
      let queryParams = `?branchId=${branchId}&startDate=${start.toISOString()}&endDate=${end.toISOString()}&type=SALE`;
      
      // Only add soldByUserId filter if a specific salesperson is selected
      if (selectedUserId && selectedUserId !== '') {
        queryParams += `&soldByUserId=${selectedUserId}`;
      }
      ...
      // Calculate earnings summary by salesperson
      const summary = {};
      ...
      setNotification({ message: err.message || 'Ma\'lumотlarni yuklashda xatolik', type: 'error' });
    } finally {
      setLoading(false);
    }
  };
  ...
  }, [selectedUserId, startDate, endDate, navigate, token, branchId]);
    ...
    <div className="ml-[255px] space-y-6 p-4">
      ...
      <div className="flex flex-col sm:flex-row sm:space-x-4 mb-6 bg-white p-4 rounded-xl shadow-sm border border-gray-100">
        ...
        <div className="flex-1 mt-4 sm:mt-0">
          ...
          <p className="text-xs text-gray-500 mt-1">
            {selectedUserId === '' ? 'Барча сотивчилар кўрсатилади' : 
             `Танланган: ${users.find(u => u.id === parseInt(selectedUserId))?.firstName} ${users.find(u => u.id === parseInt(selectedUserId))?.lastName}`}
          </p>
        </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        ...
        <ul className="space-y-6">
          {Object.entries(earningsSummary).map(([userId, summary]) => {
            ...
            // If a specific salesperson is selected, only show that one
            ...
            return (
              <li key={userId} className="border-b border-gray-200 pb-4">
                <div className="text-base font-medium text-gray-900">
                  {user.firstName} {user.lastName}
                <div className="text-sm text-gray-500 mb-2">{summary.transactionCount} ta sotuv</div>
                </div>
                <div className="space-y-1">
                  <div className="flex justify-between">
                    <span className="text-sm text-gray-600">Jami sotuv:</span>
        ...
        <p className="text-gray-500">
          {startDate && endDate ? (
            <>
              ...
              {selectedUserId ? ' tanlangan sotuvchi uchun' : ' hech qanday sotuv ma\'lumoti yo\'q'}
            </>
          ) : (
              ...
              <>
                ...
                {selectedUserId && (
                  <span className="ml-2">
                    ...
                    {users.find((u) => u.id === parseInt(selectedUserId))?.lastName}
                  </span>
                )}
              </>
            )}
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {salesData
                .filter((sale) => {
                  // If a specific salesperson is selected, only show that one's data
                  if (selectedUserId && selectedUserId !== '') {
                    return parseInt(sale.userId) === parseInt(selectedUserId);
                  }
                  // If no salesperson selected, show all data
                  return true;
                })
                .map((sale) => (
